...
class yolov5():
    ...
    def letterbox(self, im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
     ...
     # 如果new_shape是一个整数，那么将其转换为元组(new_shape, new_shape)，表示新的图像宽度和高度都应该是这个值。
        if isinstance(new_shape, int):
            new_shape = (new_shape, new_shape)

        # 计算缩放比例：r是原始图像和目标图像之间的最小宽度和高度比例。如果scaleup为False，则r不会超过1.0，这意味着图像只会被缩小，不会被放大。
        # Scale ratio (new / old)
    ...
    def detect_1(self, srcimg, i_conf):
        im = srcimg.copy()
        im, ratio, wh = self.letterbox(srcimg, self.inpWidth, stride=self.stride, auto=False)
        # Sets the input to the network
        blob = cv2.dnn.blobFromImage(im, 1 / 255.0,swapRB=True, crop=False)
