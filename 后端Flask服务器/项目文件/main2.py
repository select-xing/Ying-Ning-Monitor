...
if __name__ == "__main__":
    ...
    #model.resnet.fc = nn.Linear(2048, 10)
    ...
    best_model_path = "best_BiLSTM_model_val.pth"
    train_losses = []
    ...
    #final_model = AudioClassifier().to(device)
    #final_model.resnet.fc = nn.Linear(2048, 10).to(device)
    #final_model = models.resnet18(weights=None).to(device)
    final_model = ResNet50DualAttention().to(device)
    final_model.fc = nn.Linear(2048, 10).to(device)
